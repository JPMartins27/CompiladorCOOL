...
/** Esta class é usada para representar a árvore de herança durante o código
     geração. Você precisará preencher alguns de seus métodos e
     potencialmente estendê-lo de outras formas úteis. */
class CgenClassTable extends SymbolTable {

    /** Todas as classes no programa, representadas como CgenNode */
    private Vector nds;

    /** Este é o fluxo para o qual as instruções de montagem são geradas */
    ...
    /* bi-mapeamento entre símbolo de classe e tag */
    ...
    // Os seguintes métodos emitem código para constantes e globais
     // declarações.

    /** Emite código para iniciar o segmento .data e para
      * declarar os nomes globais
     * */
...
      * declarar os nomes globais
    ...
    /**Emite código para reservar espaço para e inicializar todos os
      * constantes Nomes de classes devem ter sido adicionados à string
      * tabela (no código fornecido, é feito durante a construção
      * do gráfico de herança) e código para emitir constantes de string
      * como efeito colateral adiciona o comprimento da string à tabela inteira.
      * As constantes são emitidas por meio da tabela de cordas e
      * inttable e produzindo código para cada entrada. */
    private void codeConstants() {
	// Adicione constantes que são necessárias pelo gerador de código.
    ...
    **/
    ...
    // Implementação muito suja, vai melhorar mais tarde
     // classe A tem f
     // classe B inh A tem B.g (A.f)
     // classe C inh B possui C.w, C.g, A.f
    ...
    /** Cria estruturas de dados representando classes Cool básicas (Object,
      * IO, Int, Bool, String). Por favor note: como é este método não
...
      * você quer.
	...
	// Alguns nomes de classes especiais são instalados na tabela de consulta
// mas não a lista de classes. Assim, essas classes existem, mas são
// não faz parte da hierarquia de herança. No_class serve como
// o pai de Object e as outras classes especiais.
// SELF_TYPE é a classe própria; não pode ser redefinido ou
// herdado. prim_slot é uma classe conhecida pelo gerador de código.
...
// A classe Object não possui uma classe parent. Seus métodos são
...
// "val" para o inteiro.
	...
	// Bool também possui apenas o slot "val".
	class_ Bool_class = 
	    new class_(0,
		       TreeConstants.Bool,
		       TreeConstants.Object_,
	...
	installClass(new CgenNode(Str_class, CgenNode.Basic, this));
   	...
   	// O seguinte cria um gráfico de herança de
     // uma lista de classes. O gráfico é implementado como
     // uma árvore de CgenNode e nomes de classes são colocados
     // na tabela de símbolos da classe base.
	...
	stringclasstag = 4 /* Mude para a tag da classe String aqui */;
	intclasstag =    2 /* Mude para sua tag de classe Int aqui */;
	boolclasstag =   3 /* Mude para sua tag de classe Bool aqui */;
        objclasstag = 0;
        ioclasstag = 1;
    ...
    /** Esse método é o coração do gerador de código. É para ser
         preenchido com atribuição de programação 5*/
    public void code() {
        setClassTags(root());
...
// Adicione seu código para emitir
...
// - etc ...
    ...
    /** Obtém a raiz da árvore de herança */
    public CgenNode root() {
	return (CgenNode)lookup(TreeConstants.Object_);
    }

    /* Precisa de alguns utilitários para encontrar recursos */
// obtém o recurso de curClass ou seu ancestral mais próximo
     // Os nomes das classes são instalados de duas maneiras. Um é nds apenas para o CgenNode
     // Dois é para symbolTable como nome (name, nd) como AbstractSymbol e nd como CgenNode
    ...
    // Obter todos os nós ancestrais de baixo para cima
